...
1. **POST /submit** validates the request (requires `text` and `creator_id`)
2. **Signal 1 — LLM (Groq):** The text is sent to `llama-3.3-70b-versatile` with a structured prompt. The model returns a 0–1 score and a one-sentence reasoning string.
3. **Signal 2 — Stylometrics:** Pure Python computes three structural metrics (sentence length variance, type-token ratio, punctuation density) and combines them into a 0–1 score.
4. **Confidence Scorer:** The two signal scores are combined with a weighted average (60/40 LLM/stylo for long text; 80/20 for short text under 100 words).
5. **Label Generator:** The combined score is mapped to one of three transparency label variants.
6. **Audit Logger:** A structured JSON entry is appended to `logs/audit.jsonl`.
7. **Response:** `content_id`, `attribution`, `confidence`, `label`, and both individual signal scores are returned to the caller.
...
**High-confidence AI (score: 0.82)**
> "Artificial intelligence represents a transformative paradigm shift in modern society. It is important to note that while the benefits of AI are numerous, it is equally essential to consider the ethical implications. Furthermore, stakeholders across various sectors must collaborate to ensure responsible deployment."

LLM score: 0.91 | Stylo score: 0.67 | Combined: **0.82** → `likely_ai`

**Low-confidence / likely human (score: 0.18)**
...
LLM score: 0.08 | Stylo score: 0.34 | Combined: **0.18** → `likely_human`

*(Scores above reflect actual test runs. Individual runs may vary slightly due to LLM temperature.)*
...
Every submission and appeal is logged to `logs/audit.jsonl` as a newline-delimited JSON entry. Sample entries:

```json
{"content_id": "a1b2c3d4-e5f6-...", "creator_id": "test-user-1", "timestamp": "2025-04-01T14:32:10.123Z", "attribution": "likely_ai", "confidence": 0.8221, "llm_score": 0.91, "stylo_score": 0.67, "status": "classified", "label": "⚠️ AI-Generated Content Likely\n...", "text_preview": "Artificial intelligence represents a transformative paradigm shift..."}
{"content_id": "b2c3d4e5-f6a7-...", "creator_id": "test-user-2", "timestamp": "2025-04-01T14:33:05.456Z", "attribution": "likely_human", "confidence": 0.1820, "llm_score": 0.08, "stylo_score": 0.34, "status": "classified", "label": "✅ Appears Human-Written\n...", "text_preview": "ok so i finally tried that new ramen place downtown..."}
{"content_id": "a1b2c3d4-e5f6-...", "creator_id": "test-user-1", "timestamp": "2025-04-01T14:34:22.789Z", "attribution": "likely_ai", "confidence": 0.8221, "llm_score": 0.91, "stylo_score": 0.67, "status": "under_review", "appeal_reasoning": "I wrote this myself from personal experience. I am a non-native English speaker.", "event_type": "appeal"}
```
